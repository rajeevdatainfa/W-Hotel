...
import 'package:hotels/model/AnnouncementsRes.dart';
...
import 'package:hotels/screens/HelpDetailsActivity.dart';
...
class HelpListActivity extends StatefulWidget {


  @override
  HelpListActivityView createState() =>HelpListActivityView();
}

class HelpListActivityView extends State<HelpListActivity> {
  double defaultFontSize = 14;
  double defaultIconSize = 17;
  bool isConnect = true;
  bool isLoaded = false;
  List<AnnouncementsDetails> _tList = [];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Announcements', style: TextStyle(fontSize: 20.0, color: Colors.white),),
        toolbarHeight: 70,
      ),
      body: SafeArea(
        child: Container(
  ...
  Widget _itemLayout(BuildContext context, int index,AnnouncementsDetails restaurentItem) {
    return Padding(
      ...
      child:  Card(
              ...
              child: GestureDetector(
                  child: InkWell(
                    onTap: () {
                      goToNextPage(context, 0,restaurentItem.file!.url!,restaurentItem.title!);
                    },
                    child: Container(
  ...
  Future getAnnouncementsList() async {
    ...
    print("request =====: " + map.toString() + "");
      ...
      try {
        var response = await http.get(Uri.parse(Apis.announcements),headers: {
          "Authorization":"Bearer "+App.getString(Cons.aToken)
        ...
        if (response.statusCode == 200) {
          ...
          try {
            ...
            AnnouncementsRes res = AnnouncementsRes.fromJson(parsed);
            setState(() {
              _tList.clear();
...
goToNextPage(BuildContext context, int intex,String url,String tName) async {
  ...
  if (intex == 0) {
    ...
    Navigator.of(context, rootNavigator: true).push(MaterialPageRoute(builder: (context) => HelpDetailsActivity(url: url,tName: tName,)));
